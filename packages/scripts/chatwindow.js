...
$(document).mouseup(function (e) {
  ...
  if (
    !emoticons_changed.is(e.target) && // if the target of the click isn't the container...
    emoticons_changed.has(e.target).length === 0
...
var messenger = (function () {
  ...
  var chatSocket = {
    ...
    onOpen: function (evt) {
      try {
        console.log('chat on open called');
      } catch (e) {
        console.dir(e);
      }
    },
    onClose: function (evt) {
      try {
        console.log('chat on clolse called');
      } catch (e) {
        console.dir(e);
      }
    },
  ...
  var prepareMessage = function (msg) {
    if (utils != 'undefined') return utils.nl2br(msg);
    else return msg.replace(/(?:\r\n|\r|\n)/g, '<br />');
  };

  var _r_pos = function () {
    // re positioning the whole chat boxes
    var align = 0,
      width = 0,
      active = '',
      z_i = 0;
    ...
    for (var x = length - 1; x >= 0; x--) {
      ...
      if (active == chatboxtitle) {
        ...
        $(prefix + chatboxtitle + ' .chatboxtextarea').addClass(
          'chatboxtextareaselected'
        );
        $(prefix + chatboxtitle + ' .chatboxcontent')
          .removeClass('border-inactive')
          .addClass('border-active');
        $(prefix + chatboxtitle + ' .chatboxinput')
          .removeClass('border-inactive')
          .addClass('border-active');
      } else {
        ...
        $(prefix + chatboxtitle + ' .chatboxtextarea').removeClass(
          'chatboxtextareaselected'
        );
        $(prefix + chatboxtitle + ' .chatboxcontent')
          .removeClass('border-active')
          .addClass('border-inactive');
        $(prefix + chatboxtitle + ' .chatboxinput')
          .removeClass('border-active')
          .addClass('border-inactive');
      }
      ...
      if (align == 0) {
        ...
        tempx = x + 1;
        var temp_box = $(
          prefix + $Boxes[drawnboxes[drawnboxes.length - 1]].title
        );
        if (temp_box.length) {
    ...
    if (popboxes.length) {
      if ($('#chatBoxMore').length) {
        ...
        $('<div/>')
          .attr('id', 'chatBoxMore')
          .addClass('dropdown dropup btn btn-warning')
          .css('left', left_point + 'px')
          .css('bottom', '0px')
          .css('position', 'fixed')
          .html(pushboxTemplate)
          .appendTo('body');
      }

      for (var k = 0; k < popboxes.length; k++) {
        var text =
          '<li role="presentation"><a role="menuitem" onclick="messenger.show(\'' +
          $Boxes[popboxes[k]].info.join(',') +
          '\');return false;" >' +
          $Boxes[popboxes[k]].display_name +
          '</a></li>';
  ...
  var fb_event_handler = function (title) {
    //blur focus hanlder
    $(prefix + title + ' .chatboxtextarea')
      .blur(function (event) {
        //event.stopPropagation();
    ...
    $(prefix + title).delegate(
      '.status_seen, .status_delivered, .status_sent',
      'mouseenter',
      function () {
  ...
  var _ccb = function (chatboxtitle, minimizeChatBox, tag_chat) {
    //create chat box
    ...
    var html = [
      '<div  class="chatboxhead active">',
      '<div class="chatboxtitle"><span class=title>' + displayName + '</span>',
      '<span class="iconclose">',
      '<a href="javascript:void(0)" onclick="javascript:messenger.closeChatBox(\'' +
        friendName +
        '\')">',
      ...
      '<span></span></a>',
      '<a href="javascript:void(0)" onclick="javascript:messenger.toggleChatBoxGrowth(\'' +
        chatboxtitle +
        '\')">',
      ...
      '<textarea  class="chatboxtextarea"  onkeydown="javascript:return messenger.checkChatBoxInputKey(event,this,\'' +
        chatboxtitle +
        "','" +
        friendName +
        '\');"></textarea>',
      '<a class="emoticonBtn emoticon_icon"  title="EMO"></a><div class="clear"></div>',
      '</div>',
    ].join('');
  ...
  var _min_box = function (chatboxtitle) {
    // initial minimize all after a redirect
  ...
  var _x_box = function (t) {
    ...
    for (var i = chatBoxes.length - 1; i >= 0; i--) {
      // index of does not support in ie so loop through all element as an templorary solution
  ...
  var fire_m = function (chatboxtitle, message, type) {
    ...
    var jObj = JSON.stringify({
      tag_chat: $(prefix + chatboxtitle).data('tag_chat'),
      friendName: $(prefix + chatboxtitle).data('tag'),
      message: message,
      type: type,
      key: key,
      timeOut: timeout,
  ...
  var _s_m = function (event, chatboxtextarea, chatboxtitle, friendName) {
    // send message on keycode 13
  ...
  var _r_m = function (key) {
    ...
    $('#' + key).html(
      '<span class="me text_deleted">This message was removed</span>'
    );
  };
  var _t_r_m = function (pid, time, remove) {
    // timeout function remove/time in second
    var init = time;
    var dosec = setInterval(function () {
      if (init === 0) {
        ...
        $('#' + pid)
          .find('.timeoutspan')
          .html(lang.time + ' : ' + time + 's');
      }
      if ($('#' + pid).length) {
        $('#' + pid)
          .find('.timeoutspan')
          .html(lang.time + ' : ' + init + 's');
        init--;
      }
    }, 1000);
  };
  var _a_m = function (ob) {
    //append message | take message object parameter
    ob = ob || {};
    var m = ob.m;
    ...
    if (ob.hasOwnProperty('type') && ob.type == 101) {
      //rewrite it from begininig
      ...
      if ($('#' + m.packetID).length) {
        $('#' + m.packetID).html(
          '<span class="you text_deleted">This message was removed</span>'
        );
      }
      return;
    }
    ...
    switch (true) {
      ...
      case m.key === '' && m.packetID !== '':
        ...
        if (m.key !== '' && $('#' + m.key).length)
          $('#' + m.key).attr('id', packetID);
        if (m.timeout > 0) {
          if (d_name == userIdentity) {
            _t_r_m(packetID, m.timeout, false);
          } else {
            if (document.hasFocus()) _t_r_m(packetID, m.timeout, true);
            else
    ...
    if ($('#' + packetID).length) {
      if (reactid !== '') {
        $('#' + packetID).data('react', reactid);
        $('#' + packetID)
          .find('.deletem')
          .data('react', reactid);
        $('#' + packetID).attr('id', reactid);
        packetID = reactid;
      }
      for (var i = 0; i < SClasses.length; i++)
    ...
    if (ob.hasOwnProperty('type') && ob.type == 2) {
      ...
      text +=
        ...
        '5/' +
        m.mg +
        '" alt=".." title="{AL}" width=\'100\' height=\'100\' /></div>';
    ...
    if (youme === 'me')
      text +=
        '<span class="icondelete"><a class="deletem" href="javascript:void(0)" ' +
        datas +
        '><span class="glyphicon glyphicon-remove-circle"></span></a></span>';
    text += '<br class="clear" />';
    if (mtime !== '') {
    ...
    if ($('#' + packetID).length) {
      if (
        $('#' + packetID).hasClass('status_sent') ||
        $('#' + packetID).hasClass('status_seen')
      ) {
      } else {
        $('#' + packetID).replaceWith(text);
      }
    } else {
      $(prefix + ob.uid + ' .chatboxcontent')
        .append(text)
        .animate(
          {
            scrollTop: $(prefix + ob.uid + ' .chatboxcontent').prop(
              'scrollHeight'
            ),
          },
          100
  ...
  var _i_sess = function () {
    // initiate active session on page reload/redirect
