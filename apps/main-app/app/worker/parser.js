(function (me) {
      ...
      for (var index = offset; index < dataView.byteLength; ) {
        ...
        switch (attribute) {
          ...
          case ATCODE.CONTACT:
            ...
            Data[attrMap[attribute]].push(
              parse(dataView.copy(index, length), 0)
            );
            break;
          default:
            length = dataView.getUint8(index++);
  ...
  function RingParser(ATTRIBUTE_CODES, parserService) {
    //jshint ignore:line
    var ReturnObject,
      packetDataStorage = {}; // for accessing from self property assigning to be returned object to a variable
    var ByteParsers = {};
    ...
    function parse(DataViewObject) {
      ...
      switch (packet_type) {
        ...
        case 2: //byte complete packet
          ...
          if (packet_type === 0) {
            // json complete packet
            return_data = DataViewObject.getString(
              header.headerLength,
              DataViewObject.byteLength - header.headerLength
            );
            try {
        ...
        case 3: //byte broken data
          ...
          if (!bp_header.key) {
            return false;
          } //returning if header key not found cause if key is not there then we can't identify broken packet
          if (!packetDataStorage[bp_header.key]) {
            // packet with this key not present in the map
          ...
          if (
            bp_header.total === packetDataStorage[bp_header.key].packetLength
          ) {
            //all packet has arrived so marge it and parse as normal header
            ...
            if (packet_type === 1) {
              ...
              return_data = full_packet.getString(
                header.headerLength,
                full_packet.byteLength - header.headerLength
              );
              try {
                return_data = JSON.parse(return_data);
                return_data = object_extend({}, return_data, header);
              ...
              return return_data
                ? object_extend({}, return_data, header)
                : false;
            }
          }
          return false;
