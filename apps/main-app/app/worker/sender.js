function buildPacketAndSend(message) {
  ...
  if (
    data.length + 2 > settings.CLIENT_DATA_SIZE &&
    message.data.actn !== 25 &&
    message.port !== 3
  ) {
    // including request_type and packet_type
    //sending as broken
    pack = changeMessageToByte(data, 1, message.port); //broken packet type is 1
    return sendBrokenPacket(
      pack,
      message.action,
      message.port,
      message.packetId
...
function sendData(data, action, packetId, force, flooding) {
  ...
  if (!flooding && floodingRequest[action].length) {
    //so its not a flooding request , we cannot allow to send new one until previous one resolved
    Logger.log(
      'already sending a same action request',
      action,
      packetId,
      'FLOODING_ERROR'
    );
  ...
  floodingData[packetId] = {
    //assigning request data object to global flooding data
    action: action,
    data: data,
    force: !!force,
...
function processSafeModeSender() {
  ...
  for (actn in floodingRequest) {
    ...
    if (floodingRequest.hasOwnProperty(actn) && floodingRequest[actn].length) {
      ...
      Logger.log(
        'current stack actn =>' + actn + ' : ' + floodingRequest[actn].length,
        'REQUEST_STACK'
      );
...
function sendCurrentActionRequests(actn, actnDataArr) {
  ...
  for (i = 0; i < actnDataArr.length; i++) {
    ...
    if (!pack) {
      ...
      return sendCurrentActionRequests(actn, actnDataArr);
    }
...
function changeMessageToByte(message, packetType, request_type) {
  ...
  var buffer,
    messageViewArray,
    index = 0,
    len = packetType ? message.length : message.length + 2;
  buffer = new ArrayBuffer(len);
  messageViewArray = new DataView(buffer);
  if (!packetType) {
    // for broken packet 1, else 0 so we have to add two byte if its complete packet
...
function keepAlive() {
  ...
  var dataTosend = new ArrayBuffer(settings.aSkey.length + 2),
    index = 2;
...
function sendBrokenPacket(messageViewArray, actn, request_type, wholePacketId) {
  ...
  var packets = [],
    ...
    full_packet,
    packet_start = 0,
    packet_end, // with full packet there are two bytes of request type and packet type sp excluding it
    len = messageViewArray.byteLength,
    header,
    headbuf,
    last_sent = Date.now(),
    now;

  var totalPacket = Math.ceil(len / settings.CLIENT_DATA_SIZE),
    packetId,
    uniqueKey =
      (settings.aSkey || settings.sKey).substr(1, 4) +
      getUniqueId(settings.tabId).toString(),
    glu = totalPacket < 128 ? 1 : 2; //one byte or two byte
  Logger.log(
    'PacketId : ' + wholePacketId + ' TotalPacket : ' + totalPacket,
    'BROKEN_PACKET'
  );
  for (i = 0; i < totalPacket; i++) {
    ...
    packetId =
      (settings.aSkey || settings.sKey).substr(-4, 4) +
      '-' +
      i +
      '-' +
      getUniqueId(settings.tabId).toString();
    ...
    hi = header.addAttributeString(
      hi,
      ATTRIBUTE_CODES.CLIENT_PACKET_ID,
      packetId
    );
    hi = header.addAttributeInt(
      hi,
      ATTRIBUTE_CODES.TOTAL_PACKET,
      glu,
      totalPacket
    );
    ...
    if (settings.aSkey) {
      hi = header.addAttributeString(
        hi,
        ATTRIBUTE_CODES.WEB_UNIQUE_KEY,
        settings.aSkey
      );
    }
    hi = header.addAttributeInt(
      hi,
      ATTRIBUTE_CODES.WEB_TAB_ID,
      1,
      settings.tabId
    );
    hi = header.addAttributeData(
      hi,
      ATTRIBUTE_CODES.DATA,
      messageViewArray,
      packet_start,
      packet_end - packet_start + 1
    );
  ...
  brokenRequest[wholePacketId].timeout = Math.max(
    Math.min(totalPacket * 1, 30 * glu),
    10
  );
...
function processBrokenRequest() {
  ...
  for (wPacketId in brokenRequest) {
    if (
      brokenRequest.hasOwnProperty(wPacketId) &&
      brokenRequest[wPacketId].packets.length
    ) {
      ...
      if (brokenRequest[wPacketId].responded) {
        ...
        Logger.log(
          'deleted : ' + wPacketId + 'Because Its responded',
          'BROKEN_PACKET'
        );
      } else if (
        !tempPacketArray.length &&
        currentTime - brokenRequest[wPacketId].rs >
          brokenRequest[wPacketId].timeout
      ) {
        delete brokenRequest[wPacketId];
        responseFalse(packet.action, wPacketId);
        Logger.log(
          'Responded False cause timeout ' +
            brokenRequest[wPacketId].timeout +
            'passes',
          'BROKEN_PACKET'
        );
        ...
        for (i = 0; i < tempPacketArray.length; i++) {
          Logger.log(
            'Starting Sub Packet ' + tempPacketArray[i],
            'BROKEN_PACKET'
          );
          if (scount > 10) {
          ...
          if (!packet) {
            ...
            Logger.log(
              'continue cause of packet not found: ',
              packet,
              'BROKEN_PACKET'
            );
            continue;
          ...
          if (packet.scount > 9) {
            //reponse false break;
            Logger.log(
              'packet tried Timeout response false',
              packet,
              'BROKEN_PACKET'
            );
            for (i = 0; i < tempPacketArray.length; i++) {
          ...
          if (currentTime - packet.lrt < 1000) {
            Logger.log(
              'continue cause last sent less then 1 sec ago : ' +
                tempPacketArray[i],
              'BROKEN_PACKET'
            );
            continue;
