angular.module('ringid.connector').factory('$$connector', $$connector);
...
function $$connector(
  ...
  $websocket,
  $$stackedMap,
  ATTRIBUTE_CODES, //jshint ignore:line
  Utils,
  $q,
  OPERATION_TYPES,
  CLIENT_DATA_SIZE,
  ...
  function resolvePendingRequest(message) {
    ...
    if (!!deferToBeResolved[message.pckId]) {
      var a = deferToBeResolved[message.pckId].resolve.call(
        deferToBeResolved[message.pckId],
        message
      );
  ...
  function broadcastUpdates(message) {
    ...
    if (subscribers.length() > 0) {
      subscribers.doForAll(function (currentCallBackObject) {
        var should_call_with,
          pattern = currentCallBackObject.filter;
        if (pattern) {
  ...
  function processOnMessageListners(json) {
    ...
    if (json.hasOwnProperty('pckId')) {
      // its a response of a request so process resolve the defer of this packet id
  ...
  function MainOnMessageHandler(message) {
    // if message has pckId then its a request response, if message has pckFs its a subscriber
    ...
    try {
      ...
      // buffer is not a arrayBuffer so throws exception
      // console.dir(e);
      return;
    }
  ...
  function init() {
    ...
    window.onbeforeunload = function () {
      RingLogger.print(
        'On before unload connection closed',
        RingLogger.tags.CONNECTION
      );
  ...
  ObjectToBeReturn = {
    ...
    close: function () {
      RingLogger.alert(
        'connection close request please see condition',
        RingLogger.tags.CONNECTION
      );
    },
    send: function (data, request_type, flooding) {
      // do not return any promise
      sendData(data, request_type, false, flooding);
    },
    request: function (data, request_type, flooding) {
      //returns a $q promise
      //if(!flooding){
    ...
    pull: function (data, request_type, flooding) {
      //returns a $q promise
      // if(!flooding){
