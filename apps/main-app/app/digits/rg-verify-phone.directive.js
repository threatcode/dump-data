angular.module('ringid.digits').directive('rgVerifyPhone', rgVerifyPhone);

rgVerifyPhone.$inject = [
  '$ringhttp',
  'settings',
  '$http',
  'Auth',
...
function rgVerifyPhone(
  ...
  function linkFunc(scope, element) {
    ...
    function onSuccess(response) {
      ...
      $http
        .post(settings.digitsServiceUrl, verifyData)
        .then(function (response) {
          if (
            response.data.phoneNumber &&
            response.data.phoneNumber.length > 1
          ) {
            var phoneNo = {
                mbl: '',
                mblDc: '',
            ...
            RingLogger.print(digitResponseNo, RingLogger.tags.PROFILE);

            for (var k = 0; k < countryList.length; k++) {
              countryCode = digitResponseNo.substr(
                0,
                countryList[k].code.length
              );
              if (countryList[k].code === countryCode) {
                phoneNo.mblDc = countryList[k].code;
                phoneNo.mbl = digitResponseNo.substr(
                  countryList[k].code.length
                );
                break;
              }
            }
            RingLogger.print(phoneNo, RingLogger.tags.PROFILE);
