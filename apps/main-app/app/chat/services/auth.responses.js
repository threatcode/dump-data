    ...
    function ChatAuthResponses($rootScope, tagChatFactory, ChatFactory,
		...
		function processUpdates(responseObject) {
			...
			if( responseObject.rc == 1111 ){
                RingLogger.alert('M: Auth Request Failed', responseObject, RingLogger.tags.CHAT)
                return;
            }
		    ...
		    if( !packetProcessorInfo ){
		        RingLogger.warning('Packet Processor Not Implemented. Action Id ', actionId, RingLogger.tags.CHAT);
		        return false;
		    }
		    ...
		    try{
		       ...
		       RingLogger.alert('Exception In PACKET Procsessing.', actionId, e , RingLogger.tags.CHAT);
		    }
		...
		function onReceivedTagChat(responseObject){
			 /* method body */
			RingLogger.print('M: Received Auth TAG CHAT Start', responseObject, RingLogger.tags.CHAT);
		...
		function onReceivedTagMemeberAdd(responseObject){
			 /* method body */
			RingLogger.print('M: Received Auth Add Member ', responseObject, RingLogger.tags.TAG_CHAT);
		...
		function onReceivedF2FChat(responseObject){
			 /* method body */
			RingLogger.print('M: Received Auth F2F CHAT Start', responseObject, RingLogger.tags.CHAT);
		...
		function onReceivedFriendMoodPresence(responseObject){
			...
			RingLogger.debug('M: Received Auth USER_MOOD_PRESENCE', responseObject, RingLogger.tags.CHAT);
			...
			if( responseObject.fndId ){
				...
				RingLogger.alert('FriendID not present in 199 Response', responseObject, RingLogger.tags.CHAT);
			}
