  ...
  return function (items, obj) {
    if (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        if (
          obj.showMutual &&
          items[i].value.friendshipStatus() !== APP_CONSTANTS.FRIEND
        ) {
          continue;
        }
        ...
        if (
          nameMatch.length === 0 ||
          items[i].value.getName().toLowerCase().indexOf(nameMatch) > -1
        ) {
  ...
  return function (items, obj) {
    if (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        //if ( nameMatch.test(items[i].value.getName())) {
        if (
          items[i].value[obj.mapApi]().toLowerCase().indexOf(nameToMatch) > -1
        ) {
          filtered.push(items[i]);
        }
