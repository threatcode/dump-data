...
var messenger = function () {
    ...
    var blinkOrder = 0;
    var prefix = '#'+ utils.getChatBoxPrefix();
            ...
            onMessage : function(evt){
                try {
                 ...
                 //   console.log("got data on chat socket >>" + data)
                    alert('chat socket onMessage data: ' + data);
    ...
    var _r_pos = function () { // re positioning the whole chat boxes
        ...
        var right_margin = (right_panel.length) ? right_panel.width() : 200;
        //var left_panel = (left_panel.length)?left_panel.width():0;
        var right = right_margin + 10;

        var left_point = left_panel.length ? left_panel.width() : 200;
        var left = left_point;
...
//                    verticalHandleClass: 'customBarHandle'});
    ...
    var _ccb = function (chatboxtitle, minimizeChatBox, tag_chat) {  //create chat box
        ...
        var html = ['<div  class="chatboxhead active">',
            ...
            '<span class="glyphicon glyphicon-remove-circle">xx</span></a></span>',
...
//                    alert('ex in im:' + exception)
    ...
    var _x_box = function (t) {
        ...
        _r_pos();
        $.post(base_url + "/secure/APISessionClose", {
            friendName: t
        }, function (data) {
        });
    };
    ...
    var _s_c_s = function(ob,options){
        ...
        var _de  = {url: base_url + "/secure/APISendMessage",type: 'post',data:{data:ob},success: function (data) {}}
    ...
    var _a_m = function (ob) { //append message | take message object parameter
        ...
        if (ob.hasOwnProperty('type') && ob.type == 2) {
            text += '<div class="chatboxmessagecontent '+ youme +' textcontent">' + prepareMessage(m.mg) + '</div>';
        }
        else if (ob.hasOwnProperty('type') && ob.type == 3) {
            text += '<div class="'+ youme +' textcontent">' + prepareMessage(utils.replaceEmoticons(m.mg)) + '</div>';;
    ...
    var _i_sess = function () { // initiate active session on page reload/redirect
        ...
        $.ajax({
            url: base_url + "/secure/APISessionStart",
            cache: false,
            dataType: "json",
...
//                console.dir(friends);
    ...
    var h = function () { //chat heartbeat
        ...
        //    var itemsfound = 0;
        $.ajax({
            url: base_url + "/secure/APISessionHeartBeat",
            cache: false,
            dataType: "json",
            success: function (data) {
                //console.log(data);
    ...
    return {
        ...
        open: function (chatuser, tag_chat) {
