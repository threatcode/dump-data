 ...
 */
    ...
    function profileHttpService($$connector, OPERATION_TYPES, Utils) { // jshint ignore:line
        ...
        // dependonUtId
        function basicAndPrivacy(utId, iscurrent) {
            var payload;
            if (!iscurrent) {
                payload = {
        ...
        function getUserDetails(obj, withMutualFriendCount) {
            var payload = {
       ...
       // NO MORE USED. USE Api.user.fetchUserPresence or UserMap.fetchPresence instead
        ...
        function fetchPresenceAndMood(futId) {
            var payload = {
        ...
        // dependonUtId
        function getUserProfileData(utId) {
            var payload;
            if (utId) {
                payload = {
        ...
        function updateProfile(userId, obj, dateObj, mobj) {
            var mObj = mobj,
                payload;
            if (mObj !== 1) {
                mObj = mObj.getTime();
        ...
        function submitNewEducation(obj) {
            var payload = {
        ...
        function updateEducationById(obj) {
            var payload = {
        ...
        function saveAinfndreqSettings(obj) {
            var payload = {
                actn: OTYPES.TYPE_CHANGE_PRIVACY,
                sn: obj.sn,
                sv: obj.sv,
        ...
        self.basicAndPrivacy = basicAndPrivacy;

        self.getUserDetails = getUserDetails;

        self.fetchPresenceAndMood = fetchPresenceAndMood;

        self.changeMood = changeMood;

        self.getMutualFriends = getMutualFriends;

        self.getUserProfileData = getUserProfileData;

        self.updateProfile = updateProfile;

        self.updateProfilePrivacy = updateProfilePrivacy;

        self.submitNewEducation = submitNewEducation;

        self.deleteEducation = deleteEducation;

        self.updateEducationById = updateEducationById;

        self.getAllSkill = getAllSkill;

        self.submitNewSkill = submitNewSkill;

        self.updateSkill = updateSkill;

        self.deleteSkill = deleteSkill;

        self.submitNewWork = submitNewWork;

        self.updateWork = updateWork;

        self.deleteWork = deleteWork;

        self.changeProfilePicture = changeProfilePicture;

        self.changeCoverPicture = changeCoverPicture;
        ...
        self.changePrivacy = changePrivacy;

        self.saveCallPvcEdit = saveCallPvcEdit;

        self.saveChatPvcEdit = saveChatPvcEdit;

        self.saveFeedPvcEdit = saveFeedPvcEdit;

        self.addSocialAccount = addSocialAccount;

        self.saveAchatSetting = saveAchatSetting;

        self.saveAinfndreqSettings = saveAinfndreqSettings;

        self.saveAfamSettings = saveAfamSettings;

        self.saveAafSetting = saveAafSetting;
    }
