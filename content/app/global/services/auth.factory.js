            ...
            function ensureLoginState() {
                ...
                if (loginData && (!window.hasOwnProperty('ringLoggedIn') || window.ringLoggedIn)) {
                    ...
                    //$rootScope.$on('AUTH.LOGIN_REQUIRED', function() {
                        //RingLogger.warning('LOGIN_REQUIRED EVENT', RingLogger.tags.AUTH);
                        //Service.loginFromLocalData();
                    //});
                    //$rootScope.$on('AUTH.LOGIN_DONE', function() {
                        //RingLogger.warning('LOGIN_DONE EVENT', RingLogger.tags.AUTH);
            ...
            function handleInvalidSession(json) {
                ...
                switch(json.actn) {
                    case OTYPES.TYPE_MULTIPLE_SESSION:
                        RingLogger.warning('MULTIPLE SESSION', RingLogger.tags.AUTH);
                        if (!loginInProgress) {
                    ...
                    case OTYPES.TYPE_INVALID_LOGIN_SESSION:
                        RingLogger.warning('INVALID LOGIN SESSION', RingLogger.tags.auth);
                    ...
                    case OTYPES.TYPE_SESSION_VALIDATION:
                        RingLogger.warning('SESSION VALIDATION', RingLogger.tags.auth);
                ...
                setTimeout(function() {
                    fetchPrivacy().then(function(json) {
                        ...
                        if (json.sucs === true) {
                            ...
                            RingLogger.warning('PERMISSION GET REQUEST FAIL', RingLogger.tags.AUTH);
                            Service._validSession = false;
                            //if (!fromLogin) {
                                //Service.loginFromLocalData();
                            //}
