;(function(global){
    ...
    var PACKET_TYPES             = Constants.PACKET_TYPES;
    var GENERAL_CONSTANTS        = Constants.GENERAL_CONSTANTS;

    var CHAT_SESSION             = CHAT_APP.CHAT_SESSION;
    var BROKEN_PACKET_CACHE      = CHAT_APP.BROKEN_PACKET_CACHE;
    ...
    var TagObject                = CHAT_APP.MODELS.TagObject;
    ...
    function processUpdates(responseObject) {
        ...
        if( !packetProcessorInfo ){
            Logger.debug('warning','Packet Processor Not Implemented. Packet Type ', packetType, "CHAT");
            return false;
        }
    ...
    function onFriendChatMsg(responseObject){      
      ...
      /* method body */
        ...
        if( messageType == GENERAL_CONSTANTS.MESSAGE_TYPES.BLANK_MESSAGE 
            && UTILS.hasChatSession( messageDate)){
        ...
        if( sendConfirmation ){
            var requestObject = ChatRequests.getFriendChatDeliveredObject(boxId.toString(), packetId);    
    ...
    function onFriendChatBrokenMsg(responseObject){      
      ...
      var packetsLength = responseObject.packetsLength;
      ...
      var requestObject = ChatRequests.getFriendChatDeliveredObject(friendId, packetId);    
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );
    ...
    function onFriendChatSeen(responseObject){      
      ...
      if( userId != getCurrentUserId() ){
          var requestObject = ChatRequests.getFriendChatSeenConfirmationObject( userId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );
      }
    ...
    function onFriendChatMultipleMsgDelete(responseObject){      
      ...
      if( userId != getCurrentUserId() ){        
          var requestObject = ChatRequests.getFriendChatMsgDeleteConfirmationObject( friendId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );          
      }
    ...
    function onTagChatTagInformation(responseObject){      
      ...
      if( !responseObject.fromHistory && !responseObject.fromOffline) {          
          var requestObject = ChatRequests.getTagChatTagInformationConfirmationObject( tagId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );   
      }
    ...
    function onTagChatTagMemberAdd(responseObject){      
      ...
      if(!responseObject.fromHistory){
          var requestObject = ChatRequests.getTagChatTagMemberAddConfirmationObject( tagId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );   
      }
    ...
    function onTagChatTagMemberAddConfirmation(responseObject){      
      ...
      var aTag = TAGS.get(tagId);
      if(!aTag){
        aTag = new TagObject(tagId);
      }

      var requestObject = REQUEST_CACHE.getChatCache(packetId, 
        {sequenceNo: responseObject.sequenceNo, packetType : PACKET_TYPES.TAG_CHAT_TAG_MEMBER_ADD }
      );
      
      requestObject.tagMembers.forEach(function(aTagMember){
        aTag.memeberUIds.push(aTagMember.userId);
      })
    ...
    function onTagChatMemberRemoveLeave(responseObject){      
      ...
      if(!responseObject.fromHistory){
          ...
          var requestObject = ChatRequests.getTagChatMemberRemoveLeaveConfirmationObject( tagId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );   
      }
    ...
    function onTagChatMemberRemoveLeaveConfirmation(responseObject){      
      ...
      var requestObject = REQUEST_CACHE.getChatCache(packetId, 
        {sequenceNo: responseObject.sequenceNo, packetType : PACKET_TYPES.TAG_CHAT_MEMBER_REMOVE_LEAVE }
      );
      ...
      var newMemberIds = [];
      for(var index = 0 ; index < aTag.memeberUIds.length; index++){
        if( !deletedMemberIdMap[aTag.memeberUIds[index]]){
          newMemberIds.push(aTag.memeberUIds[index]);
        }        
      }
      
      aTag.memeberUIds = newMemberIds;
    ...
    function onTagChatTagMemberTypeChange(responseObject){      
      ...
      if( !responseObject.fromHistory){
           var requestObject = ChatRequests.getTagChatTagMemberTypeChangeConfirmationObject( tagId, packetId );
          ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE ); 
      }
    ...
    function onTagChatMsg(responseObject){      
      ...
      var requestObject = ChatRequests.getTagChatDeliveredObject( userId, tagId, packetId );
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );
    ...
    function onTagChatMultipleMsgDelete(responseObject){      
      ...
      var requestObject = ChatRequests.getTagChatMsgDeleteConfirmationObject( tagId, packetId );
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE );
    ...
    function onTagChatGeneralBrokenPacket(responseObject){      
      ...
      var requestObject = ChatRequests.getTagChatGeneralBrokenPacketConfirmationObject(tagId, sequenceNo, packetId);
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.ONLINE);
    ...
    function onOfflineFriendUnreadMessage(responseObject){      
      ...
      if( messages.length > 0 ){
          ...
          messages.forEach(function(aMessage){
            ...
            if( aMessage.messageType != GENERAL_CONSTANTS.MESSAGE_TYPES.BLANK_MESSAGE ){
              packetIds.push({packetId : aMessage.packetId });
            }

          });

          var requestObject = ChatRequests.getOfflineFriendUnreadMessageConfirmationObject( packetIds, packetId );
    ...
    function onOfflineFriendHistoryMessage(responseObject){      
      ...
      var preProcessMessageDate = CHAT_APP.SharedHelpers.getChatServerCurrentTime();
      ...
      if( !processed ) {
          ...
          if( lowestMessageDate != preProcessMessageDate ){
              ...
              var newPacketId = UTILS.getUUIDPacketId(lowestMessageDate, true);
      ...
      var requestObject = ChatRequests.getOfflineFriendHistoryMessageConfirmationObject( friendId, packetId );
      ChatConnector.send( requestObject, CHAT_SERVER_TYPES.OFFLINE );
    ...
    function onOfflineTagInformationActivity(responseObject){      
      ...
      var requestObject = ChatRequests.getOfflineTagConfirmationObject( tagId, packetId );
      ChatConnector.send( requestObject, CHAT_SERVER_TYPES.OFFLINE );
    ...
    function onOfflineMyTagList(responseObject){      
      ...
      for(var index = 0; index < responseObject.items.length; index++){
         var anItem = responseObject.items[index];

          setTimeout(function(tagId){
              Helpers.doRequestForTagInformationWithMember( tagId).then(function(resposne){
                
                if(!resposne.sucs && !resposne.duplicate){
                  // Helpers.doRequestForTagInformationWithMember( tagId);
                }

              });

          }.bind(null, anItem.tagId ), 1000);

          var aTagObject = TAGS.get(anItem.tagId);
          if( !aTagObject){
      ...
      var requestObject = ChatRequests.getOfflineTagConfirmationObject( 0, packetId)
    ...
    function onOfflineTagUnreadMessage(responseObject){      
      ...
      var requestObject = ChatRequests.getOfflineTagUnreadMessageConfirmationObject(packetIds ,packetId)
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.OFFLINE );
    ...
    function onOfflineTagCreateTagConfirmation(responseObject){      
      ...
      var requestObject = REQUEST_CACHE.getChatCache(packetId, 
        {sequenceNo: responseObject.sequenceNo, packetType : PACKET_TYPES.OFFLINE_TAG_CREATE_TAG }
      );
      
      var tagName = requestObject.tagName;
      
      var tagMemberUserIds = [];
      requestObject.tagMembers.forEach(function(aTagMember){
        tagMemberUserIds.push(aTagMember.userId);
      })

      var aTagObject = new TagObject(tagId, tagName, tagMemberUserIds);
    ...
    function onOfflineTagHistoryMessage(responseObject){      
      ...
      for(var index = 0; index < messages.length; index++){
          ...
          if(messages.messageType == GENERAL_CONSTANTS.MESSAGE_TYPES.HISTORY_RE_FETCH) {
      ...
      if( !processed ){
          if( lowestMessageDate != preProcessMessageDate ){
              ...
              var newPacketId = CHAT_APP.UTILS.getUUIDPacketId(lowestMessageDate, true);
              Helpers.doChatHistoryMessageRequest( tagId,
                GENERAL_CONSTANTS.REQUEST_TYPES.TAG, 
                GENERAL_CONSTANTS.PAGE_DIRECTION.UP,
                newPacketId
    ...
    function onOfflineTagInformationWithMembers(responseObject){      
      ...
      var tagMemberUserIds = [];
      tagMembers.forEach(function(aTagMember){
        tagMemberUserIds.push(aTagMember.userId);
      });

      var aTagObject = TAGS.get(tagId);
      if( !aTagObject){
        aTagObject = new TagObject(tagId, tagName, tagMemberUserIds);
        TAGS.add(tagId, aTagObject);
      }else{
        aTagObject.name = tagName;
        aTagObject.memeberUIds = tagMemberUserIds;
      }
    }
    ...
    function onOfflineBrokenHistoryPacket(responseObject){      
      ...
      var requestObject = ChatRequests.getOfflineBrokenPacketConfirmationObject(sequenceNo, packetId);
      ChatConnector.send(requestObject, CHAT_SERVER_TYPES.OFFLINE );          
