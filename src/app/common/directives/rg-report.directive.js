...
function ReportController($scope,remoteData,ReportHttpService,Ringalert){
    ...
    $scope.reason = $scope.spamList[0].id;
     RingLogger.print(remoteData,RingLogger.tags.rgReport);
    // ##DIGEST_DEBUG_START##
    if(RingLogger.tags.DIGEST){
    $scope.$watch(function(){
         RingLogger.info("from ReportController",RingLogger.tags.DIGEST);
    });
    }
    // ##DIGEST_DEBUG_END##
   $scope.report = function(){
     RingLogger.print($scope.reason,RingLogger.tags.rgReport);
     if(!$scope.reason){
     ...
     ReportHttpService.reportSpam({
          ...
          rsnid : $scope.reason
           ...
           RingLogger.print(json,RingLogger.tags.rgReport);
            $scope.$close();
           if(json.sucs){
               var ob = {
					   title : 'Done!',
            ...
            return {
                ...
                link : function(scope,element,attr){
                    ...
                    function MainFunction(e){
                       RingLogger.print("clicked rgReport",RingLogger.tags.rgReport);
                    ...
                    if(RingLogger.tags.DIGEST){
                     scope.$watch(function(){
                         RingLogger.info("from rgReport",RingLogger.tags.DIGEST);
                    });
                    }
