 ...
 */
    ...
    angular.module('ringid.auth')
        ...
        SignUpController.$inject = ['Auth', '$scope', '$controller', '$boxInstance', '$authSocial', 'remoteData', 'localData', 'Storage', 'rgDropdownService'];
        function SignUpController(Auth, $scope, $controller, $boxInstance, $authSocial, remoteData, localData, Storage, rgDropdownService) { // jshint ignore:line
            ...
            function sendCode(formValid, event) {
                ...
                if(!$scope.disableForm) {
                    ...
                    Auth.sendCode($scope.credentials).then(function(json) {
                        ...
                        $scope.requestFailed(json, 'Signup:sendcode');
                    }, function(errData) {
                        $scope.requestFailed(angular.extend(errData, {sucs: false}), 'Signup:sendcode');
                });
                } else {
                }
            }
            ...
            function verifyCode(formValid, event) {
                ...
                if(!$scope.disableForm) {
                    ...
                    Auth.verifyCode($scope.credentials).then(function(json) {
                        ...
                        $scope.requestFailed(json, 'Signup:verify');
                    }, function(errData) {
                        $scope.requestFailed(angular.extend(errData, {sucs: false}), 'Signup:verify');
                    });
                } else {
                }
            }
            ...
            function signup(formValid, event) {
                ...
                if(!$scope.disableForm && $scope.credentials.password === $scope.credentials.repassword) {
                    ...
                    Auth.signup($scope.credentials).then(function(json) {
                        if(json.sucs === true) {
                            $scope.login(true);
                            //Registraion succesful. You can login now
